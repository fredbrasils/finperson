package br.com.finperson.util;

public class ConstantsMessages {

	public static final String EMAIL = "email";
	public static final String PASSWORD = "password";

	public static final String MESSAGE_ERROR_EMAIL_EXISTS = "message.error.email.exists";
	public static final String MESSAGE_ERROR_EMAIL_NOT_EXISTS = "message.error.email.not.exists";
	public static final String MESSAGE_ERROR_SEND_EMAIL_CHANGE_PASSWORD = "message.error.send.email.change.password";
	
	public static final String AUTHSIGUP_MESSAGE_ERROR_SEND_EMAIL = "authSingUp.message.error.send.email";
	public static final String AUTHSIGUP_MESSAGE_USER_REGISTERED = "authSingUp.message.user.registered";
	public static final String AUTHSIGUP_MESSAGE_USER_CONFIRMED = "authSingUp.message.user.confirmed";
	public static final String AUTHSIGUP_MESSAGE_TOKEN_RESET_PASSWORD = "authSingUp.message.token.reset.password";
	
	public static final String AUTHSIGUP_MESSAGE_PASSWORD_UPDATED = "authUpdatePassword.message.password.updated";

	public static final String AUTH_MESSAGE_ERROR_USER_OR_PASSWORD_NOT_EXISTS = "auth.message.error.user.or.password.not.exists";

	public static final String INVALID_TOKEN = "auth.message.invalidToken";
	public static final String EXPIRED_TOKEN = "auth.message.expiredToken";
	public static final String NEW_TOKEN_TO_EXPIRED_TOKEN = "auth.message.new.token.to.expired.token";
	public static final String INVALID_USER = "auth.message.invalidUser";
	public static final String INVALID_USER_ALREADY_EXISTS = "auth.message.invalid.user.exists";
	
	public static final String USER_REGISTRATION_SUBJECT = "user.registration.subject";
	public static final String USER_REGISTRATION_MESSAGE = "user.registration.message";
	public static final String USER_RESET_PASSWORD_SUBJECT = "user.resetpassword.subject";
	public static final String USER_RESET_PASSWORD_MESSAGE = "user.resetpassword.message";
	
	public static final String CATEGORY_MESSAGE_ERROR_EXISTS = "category.message.error.exists";
	public static final String SUBCATEGORY_MESSAGE_ERROR_WITHOUT_MAINCATEGORY = "subcategory.message.error.without.maincategory";
	
	public static final String SUCCESS = "operation.success";
	
	private ConstantsMessages() {
	}
}
