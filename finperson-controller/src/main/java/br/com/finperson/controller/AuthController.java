...
public class AuthController {
    ...
    @PostMapping("/signup")
    public ResponseEntity<?> registerUser(@Valid @RequestBody UserDTO signUpRequest) throws EmailExistsException {
        
    	if(userService.emailExists(signUpRequest.getEmail())) {
            return new ResponseEntity(new GenericResponse(false, "Email Address already in use!"),
        ...
        */
        UserEntity result = userService.registerNewUserAccount(signUpRequest);
        
        URI location = ServletUriComponentsBuilder
                .fromCurrentContextPath().path("/users/{username}")
                .buildAndExpand(result.getEmail()).toUri();

        return ResponseEntity.created(location).body(new GenericResponse(true, "User registered successfully"));
    }
}
